...
- Central BC (2015 to 2023)
- Northern BC AABM Summer (2015 to 2023)
- Northern BC AABM Spring/Fall (2015 to 2023)
- Northern BC ISBM (2015 to 2023) 
...
The best model for Southern BC Summer fisheries ($C_{all,summer}$) was one with a gamma distribution using a log link that included summer South Coast  ($C_{creel,summer}$), marked and kept status ($S$), finescale fishery ($F$), and an interaction between summer creel and finescale fishery, creel summer and status, and status and fishery:
...
The best model for Southern BC Spring and Fall fishery catch ($C_{all,season}$) was one with a gamma distribution with a log link which included summer creel ($c_{creel,summer}$), status ($S$), finescale fishery ($F$), season ($Z$), and various interactions of these terms:

$$ C_{all,spring/fall} = C_{creel,summer} + Z + S + C_{creel,summer}:F + C_{creel,summer}:S + F:Z $$
...
#| fig-cap: "NBC AABM annual modelled results"
...
Figure \@ref(fig:fig-heads-kept-current) shows the linear relationship between Chinook marked kept catch and heads submitted to the SHRP from 2005 to 2023.  As the $r^2$ of `summary(all_heads_lm)$r.squared` suggests, there is a strong relationship between the marked kept catch across the whole period of this analysis with an average sample rate of `format(unname(coef(all_heads_lm)[1]), digits = 3)` for all Pacific Recreational fisheries.
...
When comparing the model fit before iRec (figure \@ref(fig:fig-heads-kept-preirec)) and with iRec (figure \@ref(fig:fig-heads-kept-postirec)), CWT sample rates are fairly consistent with a before iRec average sample rate of `format(unname(coef(pre_irec_lm)[1]), digits = 3)` and an iRec period sample rate of `format(unname(coef(post_irec_lm)[1]), digits = 3)` with similar $r^2$ values.  This suggests that there is no consistent bias in the modelled mark kept catch estimated, compared to the estimates that include iRec.   
