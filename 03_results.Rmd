...
In the following section we present the results of revising Chinook catch using both the combined iREC and creel method for more recent years and modelling to revise pre-iREC years back to 2005.  In addition to revising catch, the relationship between mark kept Chinook catch and CWT sampling through the SHRP is discussed.  This includes the sample rates across all regions and time series of CWT regions.  

## Revised Chinook catch estimates (with iREC)

We found that catch estimates that included iREC data were broadly higher than previous estimates that didn't include iREC information. As an example, Figure \@ref(fig:fig-jdf-catch-revised-1) shows the effect of the summed and revised 2013 to 2023 kept catch estimates for Juan de Fuca fishery (dashed line) compared to the previous estimates without iREC inclusion (solid line). In 2023, the revised estimates were 19% higher than the estimates without iREC included. However, in 2019, the revised estimates were smaller than the previous estimates, likely due to the creel filtration process removing a biased creel estimate. The revised kept catch plots for the remaining fisheries can be found in Appendix A.
...
The best model for Southern BC Summer fisheries was one with a gamma distribution using a log link that included summer South Coast creel, marked and kept status, finescale fishery, and an interaction between summer creel and finescale fishery, creel summer and status, and status and fishery:

$$
C_{all,summer} = C_{creel,summer} + f + s + C_{creel,summer}:S + S:F
$$
...
$$ C_{all,season} = C_{creel,summer} + Z + S + C_{creel,summer}:F + C_{creel,summer}:S + F:Z $$
...
$$ C_{all,summer} = C_{historic,summer} + S + C_{historic,summer}:S $$
...
###Chosen model
summer_north_aabm_model <- sport_catch_models$model_list$north_aabm_summer
...
The best model for Northern BC ISBM Summer fisheries was one with a gamma distribution using a log link that included summer historic data (Area 3 and 4 Creel + logbook) and status, with no interaction of these terms.

$$ C_{all,summer} = C_{historic,summer} + S $$
...
$$ C_{all,Z} = C_{historic,summer} + Z + S + C_{historic,summer}:S $$
...
#| fig-cap: "NBC AABM modelled results for Spring fishery"

###Chosen model
season_north_aabm_model <- sport_catch_models$model_list$north_aabm_season
...
$$ C_{all,Z} = C_{historic,summer} + Z + S + Z:S   $$
...
As CWT stratification is based on (CWT map reference)

Figure \@ref(fig:fig-heads-kept-current) shows ...

```{r results Head to Kept Mark Catch, echo=FALSE}
#| label: fig-heads-kept-current
#| fig-cap: "Chinook heads to mark kept catch based on combined creel and iREC.  Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (ie NBC and CBC) from 2005 to 2023."
...
#| fig-cap: "Chinook heads to mark kept catch based on combined creel and iREC.  Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (ie NBC and CBC) from 2005 to 2012."
...
#| fig-cap: "Chinook heads to mark kept catch based on combined creel and iREC.  Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (ie NBC and CBC) from 2013 to 2023."
...
#| label: fig-indcwtest
#| fig-cap: "**Need to update this caption**"
...
ggplot() +
  ...
  scale_color_manual(values=c( "lightblue4", "lightgreen"),
                     labels = c("Revised estimates", "2024 CWT Estimates"))+
