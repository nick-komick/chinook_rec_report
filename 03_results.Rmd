...
#| fig-cap: "Juan de Fuca yearly kept catch estimates based on both previous unfiltered creel/log book estimates and the revised estimates that includes iREC"
...
The best model (delta AIC = X) for Southern BC Summer fisheries ($C_{total,summer}$) was one with a gamma distribution using a log-link that included summer South Coast ($C_{creel,summer}$), marked and kept status ($S$), finescale fishery ($F$), and an interaction between summer creel and finescale fishery, summer creel and status, and finescale fishery and status:
