...
We found that catch estimates that included iREC data were broadly higher than previous estimates derived from only unfiltered creel and logbook information. As an example, Figure \@ref(fig:fig-jdf-catch-revised-1) shows the effect of the summed and revised 2013 to 2023 kept catch estimates for the Juan de Fuca fishery (dashed line) compared to the previous estimates without iREC inclusion (solid line). Though the relative difference between the two time series vary, the revised estimates are consistently higher than the estimates without iREC. As the comparison plots for various fisheries shows in Appendix \@ref(app:revised-catch-timeseries), in most cases kept catch was increased with a few exceptions where there was a small reduction in specific years.
...
The best model for Southern BC Summer fisheries ($C_{total,summer}$) was identified using AIC-based model selection and was a model with a gamma distribution using a log-link that included summer South Coast ($C_{creel,summer}$), marked and kept status ($S$), finescale fishery ($F$), and an interaction between summer creel and finescale fishery, summer creel and status, and finescale fishery and status:
...
$$
C_{total,Z} = C_{creel,summer} + Z + S + C_{creel,summer}:F + C_{creel,summer}:S + F:Z, Z \in (\text{spring},\ \text{fall})
\label{eq:sbc_season}
$$
...
#| fig-cap: "Southern BC Spring and Fall fishery modelled relationship between concentrated summertime unfiltered creel estimate (months 7 and 8) and total iREC catch estimate in spring (months 1:4) and fall (10:12) months. Data points are yearly estimates from 2013 to 2023."
...
#| fig-cap: "Haida Gwaii annual fishery modelled relationship between concentrated summertime unfiltered creel estimate (months 7 and 8) and total iREC catch estimates in the whole summer period (months 5:9). Data points are yearly estimates from 2015 to 2023."
...
As the total number of heads collected by the SHRP is expected to be driven by the the total kept marked catch in recreational fisheries, the total kept marked catch and total heads collected by the SHRP should be correlated. In contrast, the SHRP relies on several sampling approaches (e.g., log sampling, public collection points, avid anglers) so should not necessarily be driven by creel survey coverage. Furthermore, SHRP has generally run consistently over the transition time between the modelled catch (pre-2013) and subsequent catch period during which iREC has been incorporated into estimates. So, the linear relationship between marked kept catch and head recoveries from both the 2005 - 2012 modelled catch timeseries and the 2013 - 2023 combined creel and iREC catch timeseries should be similar.
...
#| fig-cap: "Chinook heads to marked kept catch based on combined creel and iREC.  Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (i.e., NBC and CBC) from 2005 to 2023."
...
#| fig-cap: "Chinook heads to marked kept catch based on combined creel and iREC. Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (i.e., NBC and CBC) from 2005 to 2012."
...
#| fig-cap: "Chinook heads to marked kept catch based on combined creel and iREC.  Each point is a yearly CWT Region based on the seasonal time periods in the southern portion of BC and annual periods in the nothern portion of BC (i.e., NBC and CBC) from 2013 to 2023."
...
The time series of submission rates of for each region is shown in Figure \@ref(fig:fig-fishery-rates).  
 ...
 * Marked kept catch estimates are used to estimate CWTs
