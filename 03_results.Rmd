...
The best model for Haida Gwaii fishery was one with a Gaussian distribution historic data (Haida creel + logbook, $C_{historic}$) and status ($S$):
...
Figure \@ref(fig:fig-heads-kept-current) shows the linear relationship between Chinook marked kept catch and heads submitted to the SHRP from 2005 to 2023.  As the $r^2$ of `r summary(all_heads_lm)$r.squared` suggests, there is a strong relationship between the marked kept catch across the whole period of this analysis with an average sample rate of `r format(unname(coef(all_heads_lm)[1]), digits = 3)` for all Pacific Recreational fisheries.
