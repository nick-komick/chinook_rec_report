 ...
 * Seperate models for southern BC summer pre-terminal sport fisheries that better align to creel program delivery

All models were fit through the origin, as catch from both sources are expected to potenially report zero catch.  Also, the effort was to use a gaussian fit unless a Gamma fit was clearly a better fit.  This is mitigate potental large increases or decreases in catch when creel catch estimates are outside the range of estimates observed with the fitted data.  When evaluating model fits, we used the following criteria to select the final model form used in this analysis.
