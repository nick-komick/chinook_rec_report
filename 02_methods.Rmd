...
Central Coast catch estimation is exclusively logbook data from lodges. The PFMAs included in the Central Coast are 6 through 10, and their offshore counterparts 106 through 110 (Figure \@ref(fig:fig-PFMA)). Catch data is not typically kept by marked and kept status, so those distinctions need to be reconstructed using mark rate and release rates. Although not all lodges provide logbook catch data, the catch data has historically been reported as the total catch for the region. This was identified as a bias in the recreational catch estimates that iREC inclusion should address.
...
The calibration coefficient $m$ is estimated annually for each licence year (i.e. April 1 to March 31) using all area and month estimates of creel that are of appropriate quality based on the criteria identified in the "South Coast Creel Survey quality control" section of this document. Separate calibration factors are estimated for total kept and released catch, by species, for angling by boat. As both the creel and iREC estimates have uncertainty associated with them, an error-in-variables approach is used to estimate $m$ using JAGS [@plummer2003jags] through the `rjags` R package [@rjags]. The specific model code used is provided below, with `creel.obs` representing the creel estimate, `creel.se` representing the standard error of the creel estimate, and `iRec.obs` representing the iREC uncalibrated catch estimate for the same area and time as the creel estimate. The calibration coefficient are used to correct both angling by boat and angling from shore catch estimates.  
...
We calculate mark rate in a number of different ways and prioritized the assignment of mark rate in the order identified below, based on availability.  Only mark rates that are based on a total catch estimate $\geq 10$ are used:
...
4.  Seasonal area average - Mark rate calculated for a given source within a given year, season, and area
...
When identifying the best catch estimate between  best catch estimate between creel (augmented with logbook data) and calibrated iREC, we used different procedures for Southern BC, compared to Northern and Central BC.  This allows catch estimates to align the unique challenges in deliverying catch monitoring programs within these various regions.

In Southern BC from 2013 to present, we use the combined creel and logbook data if the creel passes quality control rules.  These rules prioritize quality-controlled creel estimates, when most creel survey effort is concentrated, and prioritizes calibrated iREC data during shoulder seasons and anywhere there is a gap in creel coverage.  Figure \@ref(fig:fig-area-catch-process) provides a diagram on how the catch data sources are combined.
...
Where iREC is unavailable (before 2013 in Southern BC and before 2015 in North/Central BC), we predict iREC-incorporated catch estimates by finescale fishery using a modelled relationship between summertime creel estimates without iREC and seasonal iREC-incorporated catch estimates from the associated years of creel data. These seasonal estimates are then rolled up into yearly estimates. This process, shown in Figure \@ref(fig:fig-before-irec-flowchart) below, is automated and accomplished in the modelling function of the `pacRecCatch` R package [@pacRecCatch].

```{r methods before irec flowchart, out.width="65%", results='asis', echo=FALSE}
#| label: fig-before-irec-flowchart
#| fig-cap: "Diagram showing the steps to identifying expand catch within all regions in years before iREC is available."
...
Following the rules for incorporating iREC, we split previous fisheries into spring (months 1:4), summer (months 5:9), and fall (10:12) seasons. Fisheries were split by season in order to better combine estimates since there are not always iREC or creel estimates for each month. We did not split terminal fisheries up by season as they typically are defined by limited time periods for a given stock.

With many of the fisheries, there is a relatively consistent partial creel and logbook coverage throughout the time series (from 2005 to 2023). This consistent period of coverage is used to predict the estimated catch in the other years where we don't have iREC. For South Coast fisheries, this data is unfiltered South Coast creel + logbook catch estimates; for North and Central coast this data is referred to as "historical" but is logbook only for Central Coast and a combination of unfiltered Haida or Area 3/4 creel and logbook catch estimates in the North.

### Modelling protocol

For this initial effort to provide a consistent time series back to 2005, we explored several different model forms to develop a relationship between the creel + logbook data and the total catch estimates.  Although we describe some of other relationships that were explored in the course of developing the models, we do not describe all the model forms explored and only present the final selected model.  This approach could be improved in the future by incorporating an automated model selection process that explores more relationships.  Several key criteria that were used when developing the model relationships include:

 * Separate models for central, and northern BC fisheries since the data inputs are different
 * Single model for southern BC spring and fall pre-terminal sport fisheries since these are iREC-only data
 * Separate models for southern BC summer pre-terminal sport fisheries that better align to creel program delivery

All models were fit through the origin, as catch from both sources are expected to potentially report zero catch.  Also, the effort was to use a gaussian fit unless a Gamma fit was clearly a better fit.  This is mitigate potential large increases or decreases in catch when creel catch estimates are outside the range of estimates observed with the fitted data.  When evaluating model fits, we used the following criteria to select the final model form used in this analysis.
 ...
 * Candidate models were compared using AIC, with a lower AIC indicating a better model fit.
...
We validated the kept catch estimates from our top candidate models by regressing both the predominantly observed catch (with iREC; 2013-2023) and the modelled catch (2005-2012) with the number of marked Chinook heads by the Salmon Head  Recovery Program (SHRP), which collects heads from recreational fisheries to recover CWTs. The slope of the linear regression between the kept marked catch and the number of heads across CWT regions and seasons should provide the average submission rate of all recreational fisheries. Since there were no substantial changes to how the SHRP is delivered over both the modelled and observed catch time periods, the slopes should be similar, with deviations potentially indicating a lack in congruence between the two catch time series.
...
To support the transition to analysing mark selective fisheries within the CTC, an initial update to catch estimates based on the methods defined in this technical report were incorporate  into the 2025 ERA.  Subsequent to that update, corrections and improvements were made to the methods and iREC data.  A table comparing the creel + log book catch, the catch estimates incorporated into the February 2025 ERA and the final estimates using the updated methods within this report is provided for kept (Table \@ref(tab:tbl-previous-kept-catch)) and released (Table \@ref(tab:tbl-previous-released-catch) catch.
...
Despite improvements in CWT recovery strata and the associated complete spatio-temporal coverage in recreational catch, in rare cases for certain strata, the combination of the marked catch estimate and head submissions may still lead to substantial submission rate outliers. Approximately 2.5% of observed CWT recoveries are associated with regions and time periods with an estimated marked catch of zero or result in extreme submission rates (i.e., less than 2% or greater than 40%), mostly in years prior to iREC (pre-2013) where we rely only on modelled catch. 
