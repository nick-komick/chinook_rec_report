...
These criteria excluded 51% of all estimates since 2005 (Figure \@ref(fig:fig-creel-criteria)). PFMA 15 and 16 had the highest proportion of estimates that failed the criteria, followed by Areas 29, 19 (Georgia Strait [GS]), and 21, which all had over 80% of estimates fail the criteria. The core summer months of July and August have the highest rate of estimates passing the criteria (\> 70%), whereas more than 75% of estimates from the winter months (October through February) failed the criteria, likely due to weather making consistent overflights difficult.
...
We assembled and combined recreational Chinook catch data by developing the `pacRecCatch`[^2] R package. This package has several functions to pull data from CREST and load in other sources manually. The package has functions that will then clean, filter, align, and combine the various survey methods to the final data set as described below.

[^2]: <https://github.com/nick-komick/pacRecCatch>
...
$$
\bar{s} = \frac{\sum_i h_i s_i}{\sigma_i h_i}
