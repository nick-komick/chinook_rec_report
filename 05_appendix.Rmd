...
Before the development of this report, updated recreational catch estimates where need from Canada to support the implementation of mark selective fisheries in the CTC Exploitation Rate Analysis for 2025.  Although most of the model structures were exactly the same, the earlier analysis had different models fit for Haida Gwaii (i.e. NBC AABM) based on separate summer and spring/fall model fit.  The predicted catch estimates based on the earlier models forms, that using a log link using the equations below, are provided in Figure \@ref(fig:fig-prev-season-catch).  The previous models used a log 

$$ C_{all,summer} = C_{historic, summer} + S + C_{historic, summer}:S $$

$$ C_{all,spring/fall} = Z + S + C_{historic, summer}:Z $$
...
$$ C_{all} = C_{historic} + S $$\label{eq:haida-new-model}
...
```{r NBC AABM Model AIC table appendix, results='asis', echo=FALSE}
#| label: tab-nbc-aabm-aic
#| fig-cap: "AIC for the two orignal modal fits (i.e. Original Summer and Original Spring/Fall) and the new annual gaussian fit."

fit_aic_df <-
  data.frame(Model = c("Original Summer (Gamma Log)", 
                       "Original Spring/Fall (Gamma Log)", 
                       "New Annual Gaussian Fit"),
