...
Since the iREC survey began in 2012, several changes have been made to how annual recreational licences are selected for inclusion in each survey month. All changes to selection procedures take effect at the start of a new licence year (April 1st). For example, the selection rate for 1-, 3-, and 5‑day (“term”) licences was 25% during licence years 2012–2019, but has been 100% since the 2020 licence year.
...
#| label: fig-wcvi-sum-residuals
#| fig-cap: "Validation plots for Westcoast Vancouver Island summer model"

res_gam <- simulateResiduals(sport_catch_models$model_list$wcvi_summer_model, 
                             plot = T, 
                             quantreg=T)
```

```{r Modelling summer subsetting appendix, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
#| label: fig-sog-sum-residuals
#| fig-cap: "Validation plots for Strait of Georgia Summer model"

res_gam <- simulateResiduals(sport_catch_models$model_list$sog_summer_model, 
                             plot = T, 
                             quantreg=T)
```
...
#| label: fig-seasonal-sog-jnst-residuals
#| fig-cap: "Validation plots for Strait of Georgia and Johnstone Strait seasonal (spring/fall) fisheries"

res_gam <- simulateResiduals(sport_catch_models$model_list$seasonal_GS_noJDF, plot = T, quantreg=T)
...
```{r Modelling CBC subsetting appendix, message=FALSE, warning=FALSE, results='hide', echo=FALSE}
...
create_facet_plot <- function(finescale_fishery_name, x_var, plot_data, model_data) {
  ...
  # Ensure 'year' is integer to avoid decimal ticks in legend
  if ("year" %in% names(filtered_data)) {
    filtered_data <- filtered_data %>% mutate(year = as.integer(year))
  }
  ...
  # Create the plot
  facet_plot <- ggplot(filtered_data, aes_string(x = x_var, y = "catch_estimate", col = "year")) +
    geom_point() +
    theme_bw() +
  ...
  for (status in unique(filtered_data$status)) {
    if (has_nonzero %>% dplyr::filter(status == !!status) %>% dplyr::pull(has_nonzero)) {
      facet_plot <- facet_plot +
        ggplot2::geom_line(
          data = filtered_model_data %>% dplyr::filter(status == !!status),
          ggplot2::aes_string(y = "fit", x = x_var),  # CHANGED: remove color mapping to status
          inherit.aes = FALSE,                        # CHANGED: so the model line doesn't pick up 'year' colors
          color = "grey40",                           # CHANGED: model line in gray
          linewidth = 0.7                             # CHANGED: optional styling
        ) +
        ggplot2::geom_ribbon(
          data = filtered_model_data %>% dplyr::filter(status == !!status),
          ggplot2::aes_string(y = "fit", x = x_var, ymin = "right_lwr", ymax = "right_upr"),
          inherit.aes = FALSE,                        # CHANGED: avoid inheriting year color mapping
          fill = "grey60",                            # CHANGED: ribbon fill in gray
          alpha = 0.15                                # CHANGED: ribbon transparency
        )
    }
  }
  
  
  # CHANGED: Use integer breaks (unique years) and a step legend so no 2020.5 appears
  if ("year" %in% names(filtered_data)) {
    year_breaks <- sort(unique(filtered_data$year))
    facet_plot <- facet_plot +
      viridis::scale_color_viridis(
        option = "C", end = 0.95,
        breaks = year_breaks,               # CHANGED: integer year ticks only
        labels = year_breaks                # CHANGED: show the exact integers
      ) +
      ggplot2::guides(
        color = ggplot2::guide_colorsteps(title = "Year")  # CHANGED: step legend (no midpoints)
      )
  }
...
## Westcoast Vancouver Island Summer fisheries

```{r forloop seasonal summer appendix, results='asis', echo=FALSE}
south_summer_model <- sport_catch_models$model_list$wcvi_summer_model
...
for (i in 1:length(fishery_name_south)){
  ...
  acr <- fishery_name_south[i]
  clean_title <- lookup_vec[acr]  # gets the full name
...
```{r forloop Georgia strait seasonal spring and fall appendix, results='asis', echo=FALSE}

south_season_model <- sport_catch_models$model_list$seasonal_GS_noJDF
...
```{r forloop Juan de Fuca seasonal spring and fall appendix, results='asis', echo=FALSE}

south_season_model <- sport_catch_models$model_list$seasonal_JDF
south_season_no_nas <- south_season_model$data
...
```
```{r forloop WCVI seasonal spring and fall appendix, results='asis', echo=FALSE}

south_season_model <- sport_catch_models$model_list$seasonal_WCVI
south_season_no_nas <- south_season_model$data
