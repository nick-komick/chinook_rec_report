...
sport_mark_rate_finescale <- wrangle_rec_catch2("pacRecCatch_db.yaml", by="area")

strat_table <- 
  sport_mark_rate_finescale %>% 
  select(finescale_fishery, area, month) %>% 
  unique() %>% 
...
models_combined <- pacRecCatch::model_rec_catch2(sport_mark_rate_finescale_combined)
...
for (i in 1:length(fishery_name_south)){
  graph_data <- models_combined_old  %>% 
...
res_gam <- simulateResiduals(models_combined$model_list$south_summer, plot = T, quantreg=T)
```
...
res_gam <- simulateResiduals(models_combined$model_list$north_aabm_summer, plot = T, quantreg=T)
...
res_gam <- simulateResiduals(models_combined$model_list$north_isbm_summer, plot = T, quantreg=T)
...
res_gam <- simulateResiduals(models_combined$model_list$south_season, plot = T, quantreg=T)
...
res_gam <- simulateResiduals(models_combined$model_list$north_isbm_season, plot = T, quantreg=T)
...
res_gam <- simulateResiduals(models_combined$model_list$central_isbm, plot = T, quantreg=T)
...
south_summer_model <- models_combined$model_list$south_summer
...
south_season_model <- models_combined$model_list$south_season
...
north_aabm_season_model <- models_combined$model_list$north_aabm_season
...
north_isbm_season_model <- models_combined$model_list$north_isbm_season
