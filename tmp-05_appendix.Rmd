...
Several advancements to how annual recreational licences are selected to report for a given survey month have occurred since the iREC survey began in 2012. When changing licence selection procedures, they always take affect at the start of a new licence year (i.e. April 1st).  For example,  1, 3 and 5 day (“term”) recreational licences, the selection rate changed from 25% of all terms during the early years of iREC (i.e. licence years 2012 to 2019). Since the 2020 licence year, 100% of term licences are selected to report.

When selecting annual licences, purchase month is expected to be correlated with how the licence holder fishes. For example, licences bought in May or June are more likely to be used to target Coho and Chinook than those bought in September and October when the purchasers are more likely intending to target shellfish and finfish other than salmon. Accordingly, with each update in the selection approach, we mitigated bias caused by differential representation of licences by purchase month as described below. 
...
for (i in 1:length(fishery_name_south)){
  graph_data <- 
    ...
    mutate(estimate_type = case_match(estimate_type,
          ...
          estimate_type = if_else(year < 2013 & estimate_type == "Combined iREC and Creel",
...
create_facet_plot <- function(finescale_fishery_name, x_var, plot_data, model_data) {
  ...
  if ("year" %in% names(filtered_data)) {
    filtered_data <- filtered_data %>% dplyr::mutate(year = as.integer(year))
  }
...
prev_version_catch <-
  ...
  mutate(is_kept = grepl("kept", status, ignore.case = TRUE),
         ...
         finescale_fishery = case_match(finescale_fishery,
                                        "CBC S" ~ "CENTRAL S",
                                        "SWCVI S FALL AABM" ~ "SWCVI AABM S FALL",
                                        "SWCVI S SPRING AABM" ~ "SWCVI AABM S SPRING",
                                        "SWCVI S SUMMER AABM" ~ "SWCVI AABM S SUMMER",
                                        "NWCVI S FALL AABM" ~ "NWCVI AABM S FALL",
                                        "NWCVI S SPRING AABM" ~ "NWCVI AABM S SPRING",
                                        "NWCVI S SUMMER AABM" ~ "NWCVI AABM S SUMMER",
                                        "SWCVI S FALL ISBM" ~ "SWCVI ISBM S FALL",
                                        "SWCVI S SPRING ISBM" ~ "SWCVI ISBM S SPRING",
                                        "SWCVI S SUMMER ISBM" ~ "SWCVI ISBM S SUMMER",
                                        "NWCVI S FALL ISBM" ~ "NWCVI ISBM S FALL",
                                        "NWCVI S SPRING ISBM" ~ "NWCVI ISBM S SPRING",
                                        "NWCVI S SUMMER ISBM" ~ "NWCVI ISBM S SUMMER",
...
version_catch_tbl |>
  ...
  csasdown::csas_table(caption = "Kept catch estimates based on just creel + logbooks, previous February 2025 version of analysis, and updated version of the method presented in this report.",
                       font_size = 8)
...
version_catch_tbl |>
  ...
  csasdown::csas_table(caption = "Released catch estimates based on just creel + logbooks, previous February 2025 version of analysis, and updated version of the method presented in this report.",
                       font_size = 8)
